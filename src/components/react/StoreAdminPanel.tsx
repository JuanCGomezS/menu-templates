...
export default function StoreAdminPanel({ slug }: { slug: string }) {
  const [status, setStatus] = useState<
    "loading" | "allowed" | "denied" | "not-found"
  >("loading");
      ...
      onAuthStateChanged(auth, async (user) => {
        ...
        if (
          ...
          window.location.assign(
            withBasePath(`/admin/store/?storeId=${cachedProfile.storeId}`),
          );
          return;
        }
  ...
  if (status === "loading")
    return <StoreAdminShell title="Cargando pedidos…" />;
  if (status === "denied")
    return (
      <StoreAdminShell title="Acceso denegado">
        <p className="mt-3 text-gray-600">
          No tienes permiso para administrar esta tienda.
        </p>
      </StoreAdminShell>
    );
