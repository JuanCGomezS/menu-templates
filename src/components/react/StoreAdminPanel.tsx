...
import {
  getStoreForAdminById,
  getStoreForAdminBySlug,
} from "../../lib/public-store-data";
import {
...
export default function StoreAdminPanel({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    loadStore.then((foundStore) => {
      if (!active) return;
      if (!foundStore) {
        setStatus(profile.role === ROLES.STOREADMIN || profile.role === ROLES.SUPERADMIN ? "not-found" : "denied");
        return;
      }
      const allowed = profile.role === ROLES.SUPERADMIN || (profile.role === ROLES.STOREADMIN && foundStore.id === profile.storeId && "slug" in foundStore && foundStore.slug === slug);
  ...
  if (sessionState === "loading" || (sessionState === "authenticated" && status === "loading"))
    return <StoreAdminShell title="Preparando pedidos"><p className="mt-4 animate-pulse text-gray-600" role="status">Comprobando sesión y acceso a la tienda…</p></StoreAdminShell>;
  if (sessionState === "error")
    return <StoreAdminShell title="No pudimos validar tu sesión"><p className="mt-3 text-gray-600">Revisa tu conexión y recarga la página.</p></StoreAdminShell>;
  if (sessionState === "missing-profile")
    return <StoreAdminShell title="Perfil incompleto"><p className="mt-3 text-gray-600">Tu cuenta no tiene un perfil de permisos.</p></StoreAdminShell>;
  if (sessionState === "anonymous") return <StoreAdminShell title="Redirigiendo al acceso" />;
  if (status === "denied")
    return <StoreAdminShell title="Acceso denegado"><p className="mt-3 text-gray-600">No tienes permiso para administrar esta tienda.</p></StoreAdminShell>;
  if (status === "not-found") return <StoreAdminShell title="Tienda no encontrada o inactiva" />;
  if (status === "error")
    return <StoreAdminShell title="No se pudo cargar la tienda"><p className="mt-3 text-gray-600">Revisa tu conexión y vuelve a intentar.</p></StoreAdminShell>;
  if (!store) return <StoreAdminShell title="Preparando pedidos" />;

  return (
    <StoreAdminShell title="Operación de tienda">
      <AdminWorkspace store={store} />
    </StoreAdminShell>
  );
}
...
function AdminWorkspace({ store }: { store: StoreAccess }) {
  ...
  return (
    <>
      <div className="mt-6 flex justify-center gap-2 rounded-xl bg-gray-100 p-1 text-sm font-bold">
        <button
          type="button"
          onClick={() => setSection("orders")}
          className={`rounded-lg px-4 py-2 ${section === "orders" ? "bg-white shadow text-gray-950" : "text-gray-500"}`}
        >
          Pedidos de hoy
        </button>
        <button
          ...
          className={`rounded-lg px-4 py-2 ${section === "history" ? "bg-white shadow text-gray-950" : "text-gray-500"}`}
        >
          Estadísticas
        </button>
      </div>
  ...
  return (
    <section className="mt-6 text-left" aria-label="Estadísticas históricas">
      <form
        ...
        className="grid gap-3 rounded-xl bg-gray-50 p-4 sm:grid-cols-3"
      >
...
function OrderCard({
  ...
  const actions: Partial<Record<OrderStatus, [string, OrderStatus]>> = {
    ...
    ready: order.type === "delivery" ? ["En camino", "out_for_delivery"] : ["Entregar", "delivered"],
    out_for_delivery: ["Entregar", "delivered"],
  };
  const action = order.status && actions[order.status];
  const trackingUrl = order.trackingCode && storeSlug
    ? `${window.location.origin}${withBasePath(`/t/${storeSlug}?pedido=${encodeURIComponent(order.trackingCode)}`)}`
    : "";
  const modality =
    order.type === "delivery"
      ? "Domicilio"
  ...
  const destination = hasDeliveryLocation
  ...
  return (
    <article className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-bold capitalize text-orange-800">
          {order.status || "sin estado"}
        </span>
      </div>
      {order.type === "delivery" && (
          ...
          {hasDeliveryLocation ? (
            <div className="mt-3 flex flex-wrap gap-2">
              <a
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-slate-950 px-3 py-2 font-semibold text-white transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2"
              >
                <span
              ...
              <a
                href={`https://www.waze.com/ul?ll=${encodeURIComponent(destination)}&navigate=yes`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-2 font-semibold text-slate-800 transition hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2"
              >
                <span
                  className="material-icons-outlined text-[18px]"
                  aria-hidden="true"
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">
              Este pedido no tiene un punto de mapa confirmado.
            </p>
          )}
        </div>
      )}
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {trackingUrl && order.customerPhone && (
          <a href={`https://wa.me/${order.customerPhone.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(`Sigue tu pedido: ${trackingUrl}`)}`} target="_blank" rel="noopener noreferrer" className="rounded-lg border border-green-200 px-3 py-2 text-sm font-bold text-green-700">Enviar seguimiento</a>
        )}
...
function StoreAdminShell({
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-950">
      <AppHeader />
      <section className="mx-auto min-h-[calc(100vh-9rem)] max-w-3xl px-4 py-10">
        <div className="w-full rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-xl">
