...
import { getHistoricalOrdersForStore, getOrdersForStoreDay, subscribeToActiveOrdersForStoreDay, transitionOrderStatus, type OrderStatus, type StoreOrder } from '../../lib/orders';
...
function OrdersQueue({ store }: { store: StoreAccess }) {
  ...
  useEffect(() => {
    ...
    setDesktopAlerts(typeof window !== 'undefined' && window.localStorage.getItem(key) === 'enabled');
    const subscribeWhenVisible = () => {
      if (document.visibilityState !== 'visible') return () => undefined;
      return subscribeToActiveOrdersForStoreDay(store.id, store.timeZone || 'America/Bogota', (incoming, initial) => {
        if (initial) {
          seenOrdersRef.current = new Set(incoming.map((order) => order.id));
          return;
        }
        const newOrders = incoming.filter((order) => !seenOrdersRef.current.has(order.id));
        incoming.forEach((order) => seenOrdersRef.current.add(order.id));
        if (!newOrders.length) return;
        setAlertCount((count) => count + newOrders.length);
        setAlertMessage(`${newOrders.length === 1 ? 'Hay un pedido nuevo' : `Hay ${newOrders.length} pedidos nuevos`}.`);
        if (window.localStorage.getItem(key) === 'enabled' && Notification.permission === 'granted') new Notification('Nuevo pedido', { body: newOrders.length === 1 ? 'Revisa la cola operativa.' : `${newOrders.length} pedidos esperan atención.` });
      }, (error) => console.warn('No se pudo escuchar pedidos nuevos:', error));
    };
    let unsubscribe = subscribeWhenVisible();
    const onVisibility = () => { unsubscribe(); unsubscribe = subscribeWhenVisible(); };
  ...
  const enablePushAlerts = async () => {
    if (!auth.currentUser) { setAlertMessage('Inicia sesión nuevamente para activar push.'); return; }
    try { await registerOrderPush(store.id, auth.currentUser); setAlertMessage('Notificaciones push activadas para este dispositivo.'); }
    catch (error) { setAlertMessage(error instanceof Error ? error.message : 'No se pudieron activar las notificaciones push.'); }
  };

  const enableDesktopAlerts = async () => {
    const key = `menu-templates:order-alerts:${store.id}`;
    if (!('Notification' in window)) { setAlertMessage('Este navegador no admite notificaciones de escritorio.'); return; }
    const permission = await Notification.requestPermission();
    if (permission === 'granted') { window.localStorage.setItem(key, 'enabled'); setDesktopAlerts(true); setAlertMessage('Notificaciones de escritorio activadas.'); }
    else setAlertMessage('El navegador bloqueó las notificaciones. Puedes habilitarlas desde sus ajustes.');
  };
  ...
  return <section className="mt-6 text-left" aria-label="Cola de pedidos del día">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-gray-500">{store.name || 'Tienda'} · zona horaria: {store.timeZone || 'America/Bogota'}</p><div className="flex gap-2"><button type="button" onClick={() => void enableDesktopAlerts()} className="rounded-full border border-gray-300 px-3 py-1 text-xs font-bold text-gray-700">{desktopAlerts ? 'Alertas activadas' : 'Alertas locales'}</button><button type="button" onClick={() => void enablePushAlerts()} className="rounded-full border border-gray-300 px-3 py-1 text-xs font-bold text-gray-700">Activar push</button></div></div>{alertMessage && <button type="button" onClick={() => { setAlertCount(0); setAlertMessage(''); }} className="mb-4 w-full rounded-xl bg-orange-50 p-3 text-left text-sm font-bold text-orange-900">{alertCount > 0 && <span className="mr-2 inline-grid h-5 min-w-5 place-items-center rounded-full bg-orange-600 px-1 text-xs text-white">{alertCount}</span>}{alertMessage}</button>
