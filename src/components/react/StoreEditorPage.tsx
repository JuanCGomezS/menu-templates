...
const INPUT_CLASS =
  "w-full rounded-xl border border-gray-300 px-3 py-2 outline-none transition focus-visible:border-orange-500 focus-visible:ring-2 focus-visible:ring-orange-100";
const COMPACT_INPUT_CLASS =
  "w-full rounded-xl border border-gray-300 px-3 py-2 text-sm outline-none transition focus-visible:border-orange-500 focus-visible:ring-2 focus-visible:ring-orange-100";
...
export default function StoreEditorPage() {
  ...
  const imagePreviewKey = useMemo(
    () =>
      productDrafts
        ...
        .map(
          (product) =>
            `${product.id}:${product.imageFile?.name}:${product.imageFile?.size}:${product.imageFile?.lastModified}`,
        )
        .join("|"),
    [productDrafts],
  );
  ...
  useEffect(() => {
    const nextPreviewUrls = productDrafts.reduce<Record<string, string>>(
      (previewUrls, product) => {
  ...
  const updateProductImage = (id: string, file?: File) => {
    ...
    if (file.size > MAX_PRODUCT_IMAGE_BYTES) {
      setError(
        `La imagen pesa ${formatFileSize(file.size)}. El máximo por producto es ${MAX_PRODUCT_IMAGE_MB} MB para cuidar el plan gratuito.`,
      );
      return;
    }
  ...
  const getStoreAdminUserId = async (email: string) => {
    ...
    for (const emailCandidate of Array.from(
    ...
    throw new Error(
      "Ese usuario debe registrarse primero desde /login para quedar como customer.",
    );
  };
  ...
  const assertUniqueSlug = async (slug: string, currentStoreId?: string) => {
    ...
    if (!snapshot.empty && snapshot.docs[0].id !== currentStoreId) {
      throw new Error(
        "Ya existe una tienda con ese slug. Usa otro slug antes de guardar.",
      );
    }
  };
    ...
    return Promise.all(
      products.map(async (product) => {
        ...
        if (!isAllowedProductImage(product.imageFile)) {
          throw new Error(
            `La imagen de ${product.name || "un producto"} debe estar en formato JPG, PNG o WebP.`,
          );
        }

        if (product.imageFile.size > MAX_PRODUCT_IMAGE_BYTES) {
          throw new Error(
            `La imagen de ${product.name || "un producto"} pesa ${formatFileSize(product.imageFile.size)}. El máximo por producto es ${MAX_PRODUCT_IMAGE_MB} MB.`,
          );
        }

        const extension =
          product.imageFile.name.split(".").pop()?.toLowerCase() || "jpg";
        const imageRef = ref(
          storage,
          `stores/${storeId}/items/${product.id}/${Date.now()}.${extension}`,
        );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (storeAdmin) {
        ...
        if (
          previousOwnerRef &&
          previousOwnerData?.role === ROLES.STOREADMIN &&
          previousOwnerData.storeId === storeId
        ) {
      ...
      window.history.replaceState(
        null,
        "",
        withBasePath(`/admin/store/?storeId=${storeId}`),
      );
      setMessageTone("done");
      setMessage(
        mode === "create"
      ...
      setError(
        err instanceof Error
          ? err.message
          : "No pudimos guardar la tienda. Revisa permisos e intenta de nuevo.",
      );
    } finally {
      setSaving(false);
  ...
  return (
    <EditorShell
      title={
        mode === "create" ? "Crear tienda" : `Editar ${form.name || "tienda"}`
      }
    >
      <Messaging
        message={message}
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                ...
                <Field label="Tipo de tienda">
                  <select
                    ...
                    value={form.type}
                    onChange={(event) =>
                      updateForm("type", event.target.value as StoreType)
                    }
                    className={INPUT_CLASS}
                    required
                  >
              ...
              <div className="mt-4 space-y-5">
                ...
                <Field label="Tema">
                  <select
                    ...
                    value={form.themeId}
                    onChange={(event) =>
                      updateForm("themeId", event.target.value)
                    }
                    className={COMPACT_INPUT_CLASS}
                  >
                    {themes.map((theme) => (
            ...
            <div id="store-editor-products" role="tabpanel">
              <div className="mt-4 space-y-5">
                <div className="rounded-2xl border border-orange-100 bg-orange-50 p-4 text-sm leading-6 text-orange-950">
                  ...
                  <p>
                    Las categorías y productos alimentan la tienda pública.
                    Stock y opciones de pedido aparecen solo si el superadmin
                    habilita esas capacidades.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={addCategory}
                    className="rounded-xl bg-gray-950 px-4 py-2 text-sm font-bold text-white transition hover:bg-gray-800"
                  >
                    Agregar categoría
                  </button>
                </div>
                ...
                {categoryDrafts.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-6 text-sm leading-6 text-gray-600">
                    Todavía no hay categorías. Agregá una para empezar a cargar
                    productos reales.
                  </div>
                ) : (
                  <div className="space-y-4">
                    {categoryDrafts.map((category) => {
                      const categoryProducts = productDrafts.filter(
                        ...
                        >
                          ...
                          <div className="mt-4 space-y-3">
                              ...
                              return (
                                ...
                                >
                                  <div className="grid gap-3 lg:grid-cols-[8rem_minmax(0,1fr)_minmax(0,1fr)_8rem_6rem_auto] lg:items-end">
                                    <label
                                      className="group block cursor-pointer rounded-2xl focus-within:outline focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-orange-500"
                                      aria-label={`Subir imagen de ${product.name || "producto"}`}
                                    >
                                      ...
                                      <span className="flex aspect-square items-center justify-center overflow-hidden rounded-2xl border border-dashed border-orange-200 bg-white text-center text-orange-700 transition group-hover:border-orange-400 group-hover:bg-orange-50">
                                        {productImagePreview ? (
                                          ...
                                          <span className="px-3 text-xs font-black leading-5">
                                            <span
                                              aria-hidden="true"
                                              className="mx-auto mb-2 flex h-9 w-9 items-center justify-center rounded-full bg-orange-100 text-lg"
                                            >
                                              ↑
                                            </span>
                                            Subir foto
                                      ...
                                      <input
                                        ...
                                        className="sr-only"
                                      />
                                      <span className="mt-1 block text-[0.68rem] font-semibold text-gray-500">
                                        JPG, PNG o WebP · máx.{" "}
                                        {MAX_PRODUCT_IMAGE_MB} MB
                                      </span>
                                    </label>
                                  ...
                                  <div className="mt-3 flex flex-wrap gap-3 text-sm font-semibold text-gray-700">
                                    ...
                                    {form.deliveryOrdering && (
                                      <label className="flex items-center gap-2">
                                        <input
                                          type="checkbox"
                                          checked={product.availableForDelivery}
                                          onChange={(event) =>
                                            updateProduct(
                                              product.id,
                            ...
                            <button
                              ...
                              className="rounded-xl border border-gray-300 px-4 py-2 text-sm font-bold text-gray-700 transition hover:border-gray-950"
                            >
                              Agregar producto
                            </button>
                          </div>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                ...
                <Field label="Plan">
                  <select
                    ...
                    value={form.plan}
                    onChange={(event) =>
                      updateForm("plan", event.target.value as PlanType)
                    }
                    className={INPUT_CLASS}
                  >
                    {PLANS.map((plan) => (
                ...
                <Field label="Asignar storeadmin por correo">
                  <input
                    ...
                    autoComplete="email"
                    spellCheck={false}
                    value={form.storeAdminEmail}
                    onChange={(event) =>
                      updateForm("storeAdminEmail", event.target.value)
                    }
                    className={INPUT_CLASS}
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-3">
                <Field label="Máx. productos">
                  <input
                    ...
                    onChange={(event) =>
                      updateForm("maxProducts", event.target.value)
                    }
                    className={INPUT_CLASS}
                  />
                </Field>
                <Field label="Máx. categorías">
                  <input
                    name="maxCategories"
                    type="number"
                    inputMode="numeric"
                    ...
                    onChange={(event) =>
                      updateForm("maxCategories", event.target.value)
                    }
                    className={INPUT_CLASS}
                  />
                </Field>
                <Field label="Máx. imágenes">
                  <input
                    name="maxImages"
                    type="number"
                    inputMode="numeric"
                    ...
                    onChange={(event) =>
                      updateForm("maxImages", event.target.value)
                    }
                    className={INPUT_CLASS}
                  />
                </Field>
    ...
    <label className="flex items-start gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold text-gray-700">
      ...
      <span>
        ...
        <span className="mt-1 block text-xs font-medium leading-5 text-gray-500">
          Se habilita por tienda y no afecta al menú simple.
        </span>
      </span>
    </label>
  );
