...
export function PublicOrderCartProvider({ store, children }: { store: PublicStore; children: React.ReactNode }) {
  ...
  return <CartContext.Provider value={{ addItem }}><>{children}</><aside className="fixed bottom-4 right-4 z-50 w-[calc(100%-2rem)] max-w-sm rounded-[1.35rem] border border-white/10 bg-[#101828] p-4 text-white shadow-[0_18px_55px_rgba(16,24,40,0.32)]" aria-label="Carrito de pedido"><button type="button" onClick={() => setMinimized((value) => !value)} className="flex w-full items-center justify-between text-left font-black"><span className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-full bg-[#ff5a1f] text-[10px] tabular-nums">01</span>Pedido <span className="tabular-nums text-[#ffb08a]">({lines.reduce((sum, line) => sum + line.quantity, 0)})</span></span><span className="text-xs font-bold uppercase tracking-[.14em] text-white/65">{minimized ? 'Abrir' : 'Minimizar'}</span></button>{!minimized && <form onSubmit={submit} className="mt-3 space-y-3"><div className="grid grid-cols-2 gap-2"><button type="button" disabled={!store.capabilities?.inStoreOrdering} onClick={() => { setMode('in_store'); setLines([]); }} className={`rounded-lg p-2 text-sm font-bold ${mode === 'in_store' ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-600'}`}>En mesa</button><button type="button" disabled={!store.capabilities?.deliveryOrdering} onClick={() => { setMode('delivery'); setLines([]); }} className={`rounded-lg p-2 text-sm font-bold ${mode === 'delivery' ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-600'}`}>Domicilio</button></div>{lines.length ? <ul className="max-h-32 space-y-1 overflow-auto text-sm">{lines.map((line) => <li key={line.item.id} className="flex items-center justify-between gap-2"><span>{line.item.name} × {line.quantity}</span><span className="flex items-center gap-1"><button type="button" onClick={() => updateQuantity(line.item.id, line.quantity - 1)} aria-label={`Quitar ${line.item.name}`}>−</button><button type="button" onClick={() => updateQuantity(line.item.id, line.quantity + 1)} aria-label={`Agregar ${line.item.name}`}>+</button></span></li>)}</ul> : <p className="text-sm text-gray-500">Agrega productos del menú.</p>}<p className="border-t border-white/15 pt-3 text-right text-sm font-black tabular-nums">Total: {formatPrice(total, store.currency)}</p><input value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Tu nombre" className="w-full rounded-lg border p-2 text-sm" required />{mode === 'in_store' ? <input value={tableNumber} onChange={(event) => setTableNumber(event.target.value)} type="number" min="1" max="999" placeholder="Número de mesa" className="w-full rounded-lg border p-2 text-sm" required /> : <><input value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} type="tel" placeholder="Teléfono" className="w-full rounded-lg border p-2 text-sm" required /><input value={deliveryAddress} onChange={(event) => setDeliveryAddress(event.target.value)} placeholder="Dirección" className="w-full rounded-lg border p-2 text-sm" required /></>}<input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Notas (opcional)" className="w-full rounded-lg border p-2 text-sm" /><button disabled={!lines.length || !mode || submitting} className="w-full rounded-lg bg-[#ff5a1f] p-3 text-sm font-bold text-white transition hover:bg-[#e74711] disabled:opacity-50">{submitting ? 'Enviando…' : 'Confirmar pedido'}</button>{trackingCode && <div className="rounded-xl bg-green-50 p-3 text-xs text-green-900"><p className="font-bold">Código: {trackingCode}</p><a className="mt-1 block break-all underline" href={trackingUrl}>Ver seguimiento</a><div className="mt-2 flex gap-2"><button type="button" onClick={() => void navigator.clipboard.writeText(trackingUrl)} className="font-bold underline">Copiar enlace</button><button type="button" onClick={() => void shareTracking()} className="font-bold underline">Compartir</button></div></div>}{notice && <p aria-live="polite" className="text-center text-xs text-gray-600">{notice}</p>}</form>}</aside></CartContext.Provider>;
}
