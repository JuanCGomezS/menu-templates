...
test('la creación directa pública se bloquea: el servidor valida horario y crea pedidos', async () => {
  await assertFails(
    setDoc(doc(publicDb(), 'stores', 'store-a', 'orders', 'publictrackingcode1234'), publicOrder()),
  );
});

test('un domicilio directo también se bloquea fuera de la Function', async () => {
  await assertFails(
    setDoc(
